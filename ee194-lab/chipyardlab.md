...
> *So far, we have been working on login servers. From this point on, we will be running some more compute-intensive commands on compute servers. Prepend all compute heavy commands (everything ran in the `vlsi/` directory & `sims/` directories) with `srun -p ee194` This submits the job to a special queue of compute servers for the class so we don't crash the login servers and mess up ongoing research work (or cause each other to lose valuable work :))* 
...
```
<your username>@bwrcrdsl-#:$chipyard/sims/vcs $ srun -p ee194 make CONFIG=RocketConfig -j16
```
...
```
<your username>@bwrcrdsl-#:$chipyard/sims/vcs $ srun -p ee194 make run-binary CONFIG=RocketConfig BINARY=$RISCV/riscv64-unknown-elf/share/riscv-tests/isa/rv64ui-p-simple
```
...
```
<your username>@bwrcrdsl-#:$chipyard/sims/vcs $ srun -p ee194 make CONFIG=TutorialNoCConfig -j16
```
...
Afterwards, you should see the `.riscv` bare-metal binaries compiled in the new build folder along with a bunch of `.dump` files which contain the corresponding disassemblies. Go back to `$chipyard/sims/vcs` and try running (prepended with `srun -p ee194`):
...
```
<your username>@bwrcrdsl-#:$chipyard $ srun -p ee194 sbt
```
...
```
<your username>@bwrcrdsl-#:$chipyard/sims/vcs $ srun -p ee194 make -j16 CONFIG=CustomAccRoCCConfig BINARY=../../generators/custom-acc-rocc/baremetal_test/functionalTest.riscv run-binary-debug
```
